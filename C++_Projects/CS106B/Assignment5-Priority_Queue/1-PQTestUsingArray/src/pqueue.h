...
/* Type used for each array entry, stores the value as a string and priority as double */
...
/* Private methods */
   int bisectSearch(double priority,int lh, int rh);  //helper function to enqueue process
   void insert(std::string value, double priority,int index); //helper function to insert new element into array
