 ...
 * Usage: state.setCurrentLine(program.getNextLineNumber())
 * --------------------------------------
 * sets the key "currentLine" in map to represent the next line that will be executed by program
 */
   void setCurrentLine(int value);

/*
 * Method: getCurrentLine
 * Usage: Statement *stmt=program.getParsedStatement(state.getCurrentLine())
 * --------------------------------------
 * gets the value associated with the "currentLine" key in map; which represents the next line that will be executed by program
 */
   int getCurrentLine();
