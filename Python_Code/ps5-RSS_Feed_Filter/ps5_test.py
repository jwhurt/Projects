...
class ProblemSet5(unittest.TestCase):
    ...
    def test8FilterStories(self):
        ...
        print 'filtered_stories =', filtered_stories
        print 'stories = ', stories

        for story in stories:
            self.assertTrue(story in filtered_stories)  #throws error if original story not in filtered story.  They all should be, due to self.tt(true trigger, always true)
        print 'testing filter again, with only false trigger in trigger list...'
        filtered_stories = filter_stories(stories, [self.ft])  #should have empty list, due to false trigger
        print 'length of filtered story list (after false trigger, should be 0): ',len(filtered_stories)
        print 'filtered_stories =', filtered_stories
        print 'stories = ', stories
